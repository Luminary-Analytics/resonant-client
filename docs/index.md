# Lumi documentation

Lumi is a coding agent that runs on your computer, with the models you
choose: Anthropic, OpenAI, OpenRouter, Codex, Claude Code, local models
through Ollama or EXO, SONN, or your company's own gateway.

## Using Lumi

- [Getting started, projects and sessions](desktop-workflow.md)
- [Models: fallbacks, roles and image descriptions](models.md)
- [Signing in to enterprise model endpoints](connection-sign-in.md)
- [Usage records, prices and budgets](usage-and-costs.md)
- [Updates: channels, pins and turning them off](updates.md)
- [Lumi on macOS](macos.md)
- [Plans](plans.md)
- [Your Lumi account and your organization's Lumi Cloud](lumi-cloud.md)

## Running Lumi for an organization

- [Organization policy](enterprise-policy.md)
- [Deploying on Windows: the MSI, Intune, Configuration Manager and Group Policy](deploy-windows.md)
- [The audit log and OpenTelemetry export](audit-log.md)
- [Running Lumi without a UI: CI and containers](headless.md)
- [GitHub pull requests](github.md)

## Extending Lumi

- [Writing a capability pack](packs.md)
- [Skills](skills.md)
- [Blender, Unity and Unreal Engine](creative-editors.md)
