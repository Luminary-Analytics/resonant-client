...
test('a plan step ended by Stop shows as stopped, not as a failure', () => {
    const app = setup(() => {});
    const endings = [];
    Object.assign(app, {
        _activeTask: {},
        removeThinking() {}, _finalizeLiveCollapsedGroup() {}, ensureStepRendered() {}, scrollToBottom() {},
        _finishCancelledTask: () => endings.push('stopped'),
        _finishActiveTask: () => endings.push('failed'),
    });
    app.handleEvent({event: 'error', message: 'Interrupted', _source: 'intent', intent_id: 'plan-1'});
    app.handleEvent({event: 'error', message: 'Ollama 500: overloaded', _source: 'intent', intent_id: 'plan-1'});
    assert.deepEqual(endings, ['stopped', 'failed']);
});
