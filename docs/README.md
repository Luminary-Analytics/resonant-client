# Documentation index

The current released baseline is [Resonant 0.18.1](v0.18.1-release-notes.md).
[Unreleased changes](unreleased.md) describe work in the checkout that has not
yet shipped. Versioned release notes and dated evaluations record their own
point in time; they are not instructions to restore an old design.

The [documentation status catalog](documentation-status.md) identifies active
guides, historical plans, and dated evidence, with links to current replacements.

## Using Resonant

- [Install, configure providers, and run](../README.md)
- [Projects, sessions, search, and provider selection](desktop-workflow.md)
- [Previews, project notes, skills, and named acceptance checks](priority-improvements.md)
- [Skills and learning](skills.md)
- [Known limitations and historical bug ledger](known-issues.md)

## Contributing

- [Shared agent/contributor instructions](../AGENTS.md)
- [Architecture and module ownership](../ARCHITECTURE.md)
- [Product and engineering priorities](agentic-harness-north-star.md)
- [Model execution and prompt architecture](model-prompt-architecture.md)
- [Durable runtime services](modern-agent-runtime.md)
- [Harness core](harness-core.md)
- [Self-improvement loop](self-improvement-loop.md)
- [Roadmap](../ROADMAP.md)

## Releasing and validation

- [Release runbook](../RELEASING.md)
- [Release pipeline architecture](release-pipeline.md)
- [0.18.1 release notes and validation](v0.18.1-release-notes.md)
- [0.18.0 release notes and validation](v0.18.0-release-notes.md)
- [SONN connection setup and API contract](sonn.md)
- [0.17.2 release notes and validation](v0.17.2-release-notes.md)
- [0.17.1 release notes and validation](v0.17.1-release-notes.md)
- [0.17.0 release notes and validation](v0.17.0-release-notes.md)
- [0.16.0 release notes](v0.16.0-release-notes.md)
- [September 5 startup evaluation](ui-performance-pass-2026-09-05.md)
- [September 8 recovery validation](recovery-validation-2026-09-08.md)

## Historical design and evaluation

Root `PLAN*.md`, dated handoffs, field observations, and older release notes
retain design rationale and measurements from earlier versions. In particular,
[whats-next.md](../whats-next.md) is an archived Command Center handoff, not the
current backlog. Check source and current guides before acting on old task lists.
