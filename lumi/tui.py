...
def print_banner(backend=None, health_info: dict = None, session: Session = None, mode: str = "",
    ...
    if session is not None and mode:
        extra = f" · {escape(notice)}" if notice else ""
        console.print(f"    [{C_MUTED}]mode[/{C_MUTED}]     [{C_TEXT}]{mode}[/{C_TEXT}]  "
                      f"[{C_DIM}]{MODE_DESCRIPTIONS[mode]}{extra}[/{C_DIM}]")
...
""",
    ...
    # it. External integrations should use `lumi.gui.server`
    ...
    if chosen == "resonant":
        ...
        if not model:
            ...
            console.print(f"  [{C_WARN}]{G_CROSS} Model '{escape(model)}' {escape(problem)}[/{C_WARN}]")
            model = _select_model_interactive(models)
    ...
    def _paste_image(event):
        ...
        try:
            from lumi.engine.clipboard import read_clipboard_image, image_size_label
            ...
            if cmd in ("/quit", "/exit", "/q"):
                ...
                if rest:
                    ...
                    try:
                        ...
                        os.chdir(previous)
                        console.print(f"  [{C_ERR}]{G_CROSS} {escape(str(e))}[/{C_ERR}]")
                    else:
                        console.print(f"  [{C_FILE}]{G_ARROW} {escape(os.getcwd())}[/{C_FILE}]")
                        _print_project_trust(session)
                else:
                    console.print(f"  [{C_FILE}]{escape(os.getcwd())}[/{C_FILE}]")
