...
| `permissions.allowed_modes` | Which of `ask`, `auto-edit`, `plan` and `bypass` people may choose. Others are hidden, and a saved default outside the list becomes the first allowed mode. Missions (**Build this roadmap**) and autonomous sessions run unattended in Full-auto (`bypass`), so without it they don't start, and one already running stops at its next step (see [orchestration specialists](modern-agent-runtime.md#orchestration-specialists)). |
