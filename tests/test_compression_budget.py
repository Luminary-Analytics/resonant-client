...
class TestShouldCompressWithModel:
    def test_cloud_model_preserves_large_history(self):
        history = _make_long_history(approx_chars=500_000 * 4)
        assert should_compress(history, model_name="deepseek-v4-flash:cloud") is False
        assert should_compress(history, model_name="glm-5.2:cloud") is False

    def test_pro_does_not_trip_halfway_through_window(self):
        # Half of the advertised 1M window remains verbatim.
        history = _make_long_history(approx_chars=500_000 * 4)
        assert should_compress(history, model_name="deepseek-v4-pro:cloud") is False

    def test_pro_trips_near_full_window(self):
        # Compression begins once input reaches the protected output reserve.
        history = _make_long_history(approx_chars=1_100_000 * 4)
    ...
    def test_model_name_overrides_explicit_max_tokens(self):
        ...
        history = _make_long_history(approx_chars=1_100_000 * 4)
        # An explicit 2M generic budget cannot override the model's 1M window.
