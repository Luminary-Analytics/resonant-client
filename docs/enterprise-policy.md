...
| `shell.rules` | Execution-policy rules (`tool_pattern`, `action` allow, prompt or deny, `arg_patterns`, `reason`). They are checked before Lumi's built-in and repository rules, so nothing can loosen them. An `allow` here doesn't skip an approval prompt: only a trusted repository's own `allow` rule does that, in Auto-edit, when no rule here denies the call or asks about it. |
