...
This implements the execution side of Anthropic's Computer Use pattern:
  Claude sends actions → we execute them → return screenshots → repeat
...
def exec_computer_type(args: dict, start: float) -> ToolResult:
    """Type text or press key combinations on the desktop."""
    try:
        import pyautogui
    except ImportError:
        return ToolResult("Error: pyautogui not installed. Run: pip install pyautogui",
...
def exec_computer_scroll(args: dict, start: float) -> ToolResult:
    """Scroll at a position on screen."""
    try:
        import pyautogui
    except ImportError:
        return ToolResult("Error: pyautogui not installed. Run: pip install pyautogui",
